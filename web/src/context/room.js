...
export const RoomJoin = async (obj) => {
  ...
  if (shadow === undefined) {
    if (globalState.adminChecker === true && roomNo !== Utility.getCookie('listen_room_no')) {
    ...
    } else if (globalState.adminChecker === true && roomNo === Utility.getCookie('listen_room_no')) {
      return Hybrid('EnterRoom', '')
    } else if (globalState.adminChecker === false) {
      if (listener === 'listener' || listener === 'clip') {
        sessionStorage.removeItem('room_active')
        return RoomJoin({roomNo: roomNo, shadow: 0})
      } else if (globalState.adminChecker === false && roomNo === Utility.getCookie('listen_room_no')) {
        return Hybrid('EnterRoom', '')
      } else {
        return dispatch(setGlobalCtxMessage({
          type: "confirm",
