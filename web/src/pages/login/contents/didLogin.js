...
const DidLogin = (props) => {
  ...
  const loginClick = () => {
    ...
    if (loginInfo.phoneNum === '' && loginInfo.password === '') {
      dispatch(setGlobalCtxMessage({
        type: "alert", msg: `아이디(핸드폰 번호)와 비밀번호를 입력하고 다시 로그인해주세요.`, callback: () => {
          inputPhoneRef.current.focus()
        }
      }))
    } else if (loginInfo.phoneNum === '' && loginInfo.password !== '') {
      dispatch(setGlobalCtxMessage({
        type: "alert", msg: `아이디(핸드폰 번호)를 입력하고 다시 로그인해주세요.`, callback: () => {
          inputPasswordRef.current.focus()
        }
      }))
    } else if (loginInfo.password === '' && loginInfo.phoneNum !== '') {
      dispatch(setGlobalCtxMessage({
        type: "alert", msg: `비밀번호를 입력하고 다시 로그인해주세요.`, callback: () => {
          inputPasswordRef.current.focus()
        }
      }))
    } else {
  ...
  const fetchPhoneLogin = async (phone, pw) => {
    ...
    if (loginInfo.result === 'success') {
      ...
      if (loginInfo.code === '-1') {
        dispatch(setGlobalCtxMessage({type: "alert", msg: `아이디(전화번호)와 비밀번호를 확인하고 다시 로그인해주세요.`}))
        ...
        dispatch(setGlobalCtxMessage({
          type: "alert", title: '달빛라이브 사용 제한', msg: `${msg}`,
          callback: () => {
            if (webview && webview === 'new') {
