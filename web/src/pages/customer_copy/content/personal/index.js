  ...
  return (
    <div className="personalWrap">
      <div className="personalWrap__headerButton">
        <button onClick={() => handleHistory("personal")} className={path.includes('qnaList') ? '' : 'on'} >1:1 문의 작성</button>
        <button onClick={() => handleHistory("qnaList")} className={path.includes('qnaList') ? 'on' : ''} >나의 문의 내역</button>
      </div>
      <Switch>
