  ...
  async function fetchData() {
    const res = await Api.center_qna_add({
      data: {
        ...
        contents: content,
        email: email,
        questionFile: questionFile[0] !== false ? questionFile[0].path : "",
  ...
  return (
    <div className="personalAddWrap">
      ...
      <div className="personalAddWrap__content">
        <input type="text" className="personalAddWrap__input" placeholder="이메일 주소를 입력하세요." onChange={e => setEmail(e.target.value)} />
        <div className="personalAddWrap__caption">※ 1:1 문의 답변은 입력하신 E-mail주소로 발송 됩니다.</div>
        <input type="text" className="personalAddWrap__input" placeholder="글의 제목을 입력하세요." onChange={e => setTitle(e.target.value)} />
        <textarea className="personalAddWrap__input personalAddWrap__input--textarea" cols="30" rows="10" placeholder="작성하고자 하는 글의 내용을 입력해주세요." onChange={e => setContent(e.target.value)} />
        {
