  ...
  return(
    ...
    <div className="faqWrap">
      <div className="searchWrap">
        <input type="text" id="search" value={Store().search} placeholder="검색어를 입력해 보세요" onChange={e => Store().action.updateSearch(e.target.value)} />
        <button className="searchBtn" onClick={fnSearch} />
      </div>
      {
        Store().searching &&
        <span className="searchResult">'{Store().currentSearch}'으로 검색된 결과입니다.</span>
      }
      {
        normalList.length > 0 &&
