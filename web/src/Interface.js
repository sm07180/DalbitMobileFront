  ...
  function pushBack(event) {
    ...
    switch ((push_type + '').trim()) {
      ...
      case '31': //-----------------마이페이지>팬 보드
        ...
        if (mem_no != undefined) {
          if (isLogin) {
            if (globalState.profile.memNo === mem_no) {
              window.location.href = `/mypage/${globalState.profile.memNo}/fanboard`
            } else {
      ...
      case '48': //-----------------마이클립
        if (isLogin) {
          window.location.href = `/mypage/${globalState.profile.memNo}/my_clip`
        }
        break
      case '50': //-----------------직접입력 URL
