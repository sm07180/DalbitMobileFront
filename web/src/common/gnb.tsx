              ...
              {gnbTypes.map((item, index) => {
                return (
                  <li key={index} data-url={item.url}
                      className={`${activeType === item.url ? 'active' : (activeType === "/rankDetail/DJ" || "/rankDetail/FAN" || "/rankDetail/CUPID") && item.url === "/rank" ? "active" : ''} ${(activeType !== item.url || item.isUpdate) ? 'cursorPointer' : ''}`}
                      onClick={() => {
